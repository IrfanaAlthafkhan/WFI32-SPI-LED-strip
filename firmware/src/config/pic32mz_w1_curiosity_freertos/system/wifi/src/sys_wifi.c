...
) 
{
    switch (currentState) 
    {
        case WDRV_PIC32MZW_CONN_STATE_CONNECTED:
        {
            ...
            appData.staconnected = true ;
#if 0         
            if((appData.tmr2Expired == true)&&(appData.staconnected == true))
            {
               appData.staconnected = false ;
                appData.tmr2Expired = false;
               //while(!(SYS_WIFI_SUCCESS == SYS_WIFI_CtrlMsg(g_sysObj.syswifi , SYS_WIFI_DISCONNECT, NULL, 0)));
               
                SYS_CONSOLE_PRINT("***********************************\r\n");
                if(SYS_WIFI_CtrlMsg(sysObj.syswifi, SYS_WIFI_DISCONNECT, &g_wifiSrvcConfig, sizeof(g_wifiSrvcConfig)) == SYS_WIFI_SUCCESS)


         {   

//LED1_Toggle();
          SYS_CONSOLE_PRINT("WiFi Disconnected by STA from AP \r\n");


         }
                
                  //after disconnect from AP
                   g_wifiSrvcConfig.staConfig.autoConnect = 1;
                
         
                /* trigger reconnect to AP  periodically */
        ...
        if (&g_wifiSrvcObj == (SYS_WIFI_OBJ *)object) 
        {
            switch (event) 
            {
                ...
                case SYS_WIFI_DISCONNECT:
                {
                    ...
                    g_wifiSrvcConfig.staConfig.autoConnect = 1;//0
